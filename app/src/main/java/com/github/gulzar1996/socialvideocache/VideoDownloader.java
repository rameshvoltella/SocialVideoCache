...
public class VideoDownloader extends AsyncTask<String, Integer, Void> {
    ...
    public VideoDownloader(Activity activity,String ID) {
        mActivity=activity;
        mID=ID;
    }
    ...
    protected Void doInBackground(String... params) {
        ...
        try {
            final FileOutputStream out = new FileOutputStream(new File(mActivity.getCacheDir(),mID));
    ...
    protected void onPostExecute(Void aVoid) {
        ...
        Log.w("download", "Done");
        Toast.makeText(mActivity, "Buffered "+(readb / 1024) + "Kb", Toast.LENGTH_SHORT).show();
        if (readb>=fileLength && fileLength!=-1){
            Toast.makeText(mActivity, "Buff Complete ", Toast.LENGTH_SHORT).show();
